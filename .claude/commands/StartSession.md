...
6. **Run** version check (include in the same parallel batch):
   ```bash
   LOCAL=$(cat .claude/VERSION 2>/dev/null || echo "unknown"); REMOTE=$(curl -sf --max-time 2 https://raw.githubusercontent.com/RedBlueAI/claude-workflow/main/base/VERSION || echo ""); echo "$LOCAL|$REMOTE"
   ```
